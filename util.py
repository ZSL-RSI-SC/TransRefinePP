    ...
    def pre_refine_features(self, model, dataloader, device, extract_batch_size=50):
        ...
        with torch.no_grad():
            for features in train_seen_loader:
                ...
                #print("obtaining transformer features")
                #Hs = model.forward_feature_transformer(features)['Hs']
                Hs = model.forward_feature_transformer(features)['Hs']
        ...
        tsf = np.array(test_seen_feature)
        tuf = np.array(test_unseen_feature)
        tf = np.array(train_feature)
        #tl = np.array(train_label)
        #scipy.io.savemat('refined_features.mat', {'test_seen_features':tsf, 'test_unseen_features':tuf, 'train_feature':tf})
